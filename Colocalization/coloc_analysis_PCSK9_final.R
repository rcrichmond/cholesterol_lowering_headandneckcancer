...
####################################################################
## Read in regional snps data from #2 #
####################################################################

exposure <- as.data.frame(fread("regional_exposure_rs11591147.txt"))

## Outcome - read in for overall OC/OPC ##

outcome <- as.data.frame(fread("regional_outcome_rs11591147.txt"))
...
#coloc_results <- coloc.abf(common_dataset_exposure, common_dataset_outcome , MAF = common_dataset_exposure$MAF)
...
write.table(coloc_results_summary, "coloc_results_summary_rs11591147_final.txt", quote=F, row.names=T, sep="\t")

write.table(coloc_results_all, "coloc_results_all_rs11591147_final.txt", quote=F, row.names=F, sep="\t")
